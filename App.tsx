    ...
    <div className="flex flex-col min-h-screen max-w-xl mx-auto pb-32">
      ...
      <div className="tg-card !mt-0 !mb-2 flex justify-between gap-4">
          ...
          <div className="flex-1 text-right">
            ...
            <p className="font-bold text-sm">{dailyNorm.toFixed(0)}г</p>
          </div>
      </div>
      ...
      <div className="tg-card space-y-3">
        ...
        <div className="flex justify-between text-[11px] text-[var(--tg-theme-hint-color)] font-medium">
          ...
          <span>{mealsLeftToday > 0 ? `Еще ${mealsLeftToday} по ${nextMealAmount}г` : 'Норма выполнена!'}</span>
        </div>
      </div>
      ...
      <div className="tg-card !mt-0 space-y-4">
        ...
        <div className="flex gap-2">
          {selectedFood === FoodType.POUCH ? (
            <div className="flex-1 flex items-center justify-center bg-[var(--tg-theme-secondary-bg-color)] rounded-xl px-4 font-bold text-sm text-[var(--tg-theme-hint-color)]">1 пауч</div>
          ) : (
            <input type="number" inputMode="decimal" value={amount} onChange={(e) => setAmount(e.target.value)} placeholder="Грамм" className="flex-1 min-w-0 bg-[var(--tg-theme-secondary-bg-color)] border-none rounded-xl py-3 px-4 outline-none font-bold text-lg" />
          )}
          <button onClick={addFeeding} disabled={selectedFood !== FoodType.POUCH && !amount} className={`tg-button-main px-8 rounded-xl shrink-0 ${selectedFood !== FoodType.POUCH && !amount ? 'opacity-50 grayscale' : ''}`}>ОК</button>
        </div>
      </div>

      {/* History */}
      <div className="px-4 mt-4 mb-2 text-[var(--tg-theme-hint-color)] text-[11px] uppercase font-bold ml-2">Сегодня</div>
          ...
          <div key={log.id} className="flex items-center justify-between p-4 bg-[var(--tg-theme-bg-color)]">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-[10px] text-[var(--tg-theme-hint-color)]">{formatTime(log.timestamp)} • {log.type === FoodType.POUCH ? '1 шт' : `${log.amount}г`}</p>
              </div>
            </div>
            <div className="flex items-center gap-4">
              <span className="font-bold text-sm">+{log.equivalentGrams.toFixed(0)}г</span>
              <button onClick={() => updateStateAndSync({...state, history: state.history.filter(l => l.id !== log.id)})} className="text-red-400 p-1"><Trash2 size={16} /></button>
            </div>
          </div>
        ))}
      </div>
      ...
      {showSettings && (
        <div className="fixed inset-0 z-[110] flex flex-col justify-end bg-black/60 backdrop-blur-sm">
          <div className="relative bg-[var(--tg-theme-bg-color)] rounded-t-[24px] p-6 shadow-2xl">
            <h2 className="text-xl font-bold mb-2">Настройка семьи</h2>
            <p className="text-[var(--tg-theme-hint-color)] text-xs mb-6">Введите общее имя с женой, чтобы видеть одни данные.</p>
            <input type="text" value={familyId} onChange={(e) => setFamilyId(e.target.value.toLowerCase().replace(/\s/g, '-'))} className="w-full text-center text-2xl font-black bg-[var(--tg-theme-secondary-bg-color)] rounded-2xl py-4 outline-none mb-6" placeholder="ozzy-family" />
            <button onClick={saveSettings} disabled={!familyId} className="w-full tg-button-main py-4 rounded-xl text-lg font-bold disabled:opacity-50">Сохранить и войти</button>
          </div>
        </div>
      )}
      ...
      {showWeightModal && (
        <div className="fixed inset-0 z-[100] flex flex-col justify-end bg-black/50 backdrop-blur-[2px]">
          ...
          <div className="relative bg-[var(--tg-theme-bg-color)] rounded-t-[24px] p-6 shadow-2xl">
            ...
            <div className="flex justify-between items-center mb-6"><h2 className="text-xl font-bold">Вес (кг)</h2><button onClick={() => setShowWeightModal(false)} className="bg-[var(--tg-theme-secondary-bg-color)] p-1.5 rounded-full"><X size={20} /></button></div>
            <input type="number" step="0.01" value={newWeight} onChange={(e) => setNewWeight(e.target.value)} autoFocus className="w-full text-center text-4xl font-black bg-[var(--tg-theme-secondary-bg-color)] rounded-2xl py-6 outline-none mb-8" placeholder="0.00" />
            <button onClick={updateWeight} className="w-full tg-button-main py-4 rounded-xl text-lg font-bold">Обновить вес</button>
          </div>
        </div>
      )}
      ...
      <div className="fixed bottom-4 left-4 right-4 bg-[var(--tg-theme-bg-color)] border border-[var(--tg-theme-secondary-bg-color)] p-3 rounded-2xl shadow-xl flex items-center justify-between z-40">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-[10px] uppercase font-bold text-[var(--tg-theme-hint-color)]">Рекомендация</p>
            <p className="text-xs font-bold">{remainingToday > 0 ? `Порция: ${nextMealAmount}г` : 'Норма дня выполнена'}</p>
          </div>
        </div>
